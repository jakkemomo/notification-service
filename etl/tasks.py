...
def get_template_by_name(message: MessageIn) -> EmailMessage:
    """
...
def enrich_email_with_template_vals(message: MessageIn):
    ...
    message.body_text = HTMLFilter.convert_html_to_text(message.body_html)
...
def process_email_query(payload: dict) -> None:
    ...
    :return: None.
    """
    in_message = MessageIn(**payload)

    emails = get_email_addresses_from_ids(in_message.recipients)
    in_message.recipients = emails

    email_message = get_template_by_name(in_message)
